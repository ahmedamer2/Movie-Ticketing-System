...
public class TheSeatView {
    ...
    public TheSeatView(ArrayList<Seat> s) {
        seats = s;
    }
    ...
    // Movie theMovie = new Movie("Movie 1", "genre 1", "synopsys",
    // "posterLocation");
    // ShowTime st = new ShowTime(LocalDateTime.now(), 5, 10, theMovie);

    // for (int i = 0; i < st.getRowCapacity(); i++) {
    // for (int j = 0; j < st.getColCapacity(); j++) {
    // Seat seat = new Seat(i, j, 20, st);
    // sList.add(seat);
    // }
